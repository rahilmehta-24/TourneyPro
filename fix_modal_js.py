...
"""
    # Replace old JS
    pattern_js = r'let currentManageMatchId = null;.*?(?=function validateWinnerSelected)'
    content = re.sub(pattern_js, new_js, content, flags=re.DOTALL)
    
    # Update HTML Actions (remove modalActionsDefault and old submit buttons, create one unified action bar)
    html_actions_pat = r'<div class="glass-modal-actions" id="modalActionsDefault">.*?</form>'
    
    unified_html = """<form id="reportForm" style="display: none; margin-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1.5rem;">
        ...
        <div class="glass-modal-actions" style="justify-content: flex-end; gap: 1rem; margin-top: 2rem;">
            ...
            <button type="button" class="glass-btn-primary" onclick="saveMatchManagement()">Save Changes</button>
        </div>"""
        
    content = re.sub(html_actions_pat, unified_html, content, flags=re.DOTALL)
