...
def update_file(filename, is_tournament=False):
    ...
    if '''onclick="openMatchModal({{ match.id }}, '{{ match.status }}', '{{ match.scheduled_time.strftime('%Y-%m-%dT%H:%M') if match.scheduled_time else '' }}')"''' in content:
    ...
    if re.search(report_modal_pattern, content, flags=re.DOTALL):
        content = re.sub(report_modal_pattern, '', content, flags=re.DOTALL)
        print("Deleted reportModal.")
    else:
        print("reportModal not found.")
        
    # 3. Delete match-actions div for old report modal
    match_action_pat = r'{% if current_user and current_user\.role in \[\'admin\', \'superadmin\'\] %}\s*<div class="match-actions">\s*<button\s*onclick="openReportModal[^>]+>Report Result</button>\s*</div>\s*{% endif %}'
    if re.search(match_action_pat, content, flags=re.DOTALL):
        content = re.sub(match_action_pat, '', content, flags=re.DOTALL)
        print("Deleted match-actions.")
    else:
        print("match-actions not found.")
    ...
    content = re.sub(r'function openReportModal.*?\n    }', '', content, flags=re.DOTALL)
    content = re.sub(r'function closeReportModal.*?\n    }', '', content, flags=re.DOTALL)
    content = re.sub(r'window\.onclick = function \(event\).*?\n    }', '', content, flags=re.DOTALL)
        
    # 5. Replace new JS and Modal
    ...
    <div class="glass-modal" style="max-height: 90vh; overflow-y: auto;">
        ...
        <form id="reportForm" method="POST" style="display: none; margin-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1.5rem;">
            ...
            <div class="glass-modal-actions" style="justify-content: flex-end; gap: 1rem; margin-top: 2rem;">
                ...
                <button type="submit" name="action" value="complete_match" class="glass-btn-primary" onclick="return validateWinnerSelected()">Complete Match</button>
            </div>
        </form>
    </div>
</div>
"""
    if re.search(pattern, content, flags=re.DOTALL):
        ...
        print("Replaced JS and Modal successfully.")
    else:
...
update_file('app/templates/category/view.html', is_tournament=False)
update_file('app/templates/tournament/view.html', is_tournament=True)
